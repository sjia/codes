...
class WindowManager(object):
    ...
    def _select_by_url(self, browser, criteria):
        if "%" in criteria:
            self._select_matching_url(
                browser,
                lambda url: criteria.strip('%').lower() in url.lower(),
                "Unable to locate window with URL contains '" + criteria + "'")
        else:
            self._select_matching_url(
                browser,
                lambda url: criteria.lower() == url.lower(),
    ...
    def _select_matching_url(self, browser, matcher, error):
        try:
            starting_handle = browser.get_current_window_handle()
        except:pass
        for handle in browser.get_window_handles():
            try:
                browser.switch_to_window(handle)
                if matcher(browser.get_current_url()):
                    return
            except:pass
